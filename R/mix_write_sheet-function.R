...
mix_write_sheet <- function(data, ss, sheet = NULL, ...) {
  ...
  export_override <- Sys.getenv('MIXTAPE_EXPORT_BUCKET_OVERRIDE', "")
  if (export_override != "") {
    # Get script name from environment
    script_name <- Sys.getenv('CURRENT_SCRIPT_NAME', "unknown")
  ...
  gs_override <- Sys.getenv('MIXTAPE_GS_OVERRIDE_ID', "")
  if (gs_override != "") {
